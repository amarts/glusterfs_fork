                 ...
                 struct iovec *rpchdr, uint64_t callid)
{
        ...
        au.lk_owner.lk_owner_len = call_frame->root->lkowner_len;

        if (au.groups.groups_len)
                au.groups.groups_val = (uint16_t *)call_frame->root->groups;

        if (call_frame->root->lkowner_len)
                au.lk_owner.lk_owner_val = call_frame->root->lk_owner;
        else {
        ...
        gf_log (clnt->conn.trans->name, GF_LOG_TRACE, "Auth Info: pid: %u, uid: %d"
                ", gid: %d, owner: %s (%d)",
                au.pid, au.uid, au.gid, au.lk_owner.lk_owner_val,
