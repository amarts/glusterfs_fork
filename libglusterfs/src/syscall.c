...
#define FS_ERROR_LOG(result)                                                   \
        do {                                                                   \
                gf_msg_callingfn ("FS", GF_LOG_CRITICAL, errno,                \
...
#define FS_RET_CHECK_ERRNO(_ret)                                       \
({                                                                     \
...
{
        return FS_RET_CHECK0(lstat (path, buf));
}
...
{
        return FS_RET_CHECK0(stat (path, buf));
}
...
{
        return FS_RET_CHECK0(fstat (fd, buf));
}
...
sys_fstatat(int dirfd, const char *pathname, struct stat *buf, int flags)
{
        ...
        if(flags & AT_SYMLINK_NOFOLLOW)
                return FS_RET_CHECK0(lstat(pathname, buf));
        else
                return FS_RET_CHECK0(stat(pathname, buf));
#else
        return FS_RET_CHECK0(fstatat (dirfd, pathname, buf, flags));
#endif
}
...
{
        return FS_RET_CHECK(sys_openat(AT_FDCWD, pathname, flags, mode));
}
...
int sys_mkdirat(int dirfd, const char *pathname, mode_t mode)
{
        ...
        if(fchdir(dirfd) < 0)
                return -1;
        return FS_RET_CHECK0(mkdir(pathname, mode));
#else
        return FS_RET_CHECK0(mkdirat (dirfd, pathname, mode));
#endif
}
...
{
        return FS_RET_CHECK(readlink (path, buf, bufsiz));
}
...
{
        return FS_RET_CHECK0(closedir (dir));
}
...
{
        return FS_RET_CHECK0(mknod (pathname, mode, dev));
}
...
sys_unlink (const char *pathname)
{
        ...
        return FS_RET_CHECK0(solaris_unlink (pathname));
#endif
        return FS_RET_CHECK0(unlink (pathname));
}
...
{
        return FS_RET_CHECK0(rmdir (pathname));
}
...
{
        return FS_RET_CHECK0(symlink (oldpath, newpath));
}
...
{
#ifdef GF_SOLARIS_HOST_OS
        return FS_RET_CHECK0(solaris_rename (oldpath, newpath));
#endif
        return FS_RET_CHECK0(rename (oldpath, newpath));
}
...
#else
        return FS_RET_CHECK0(link (oldpath, newpath));
#endif
}
...
{
        return FS_RET_CHECK0(chmod (path, mode));
}
...
{
        return FS_RET_CHECK0(fchmod (fd, mode));
}
...
{
        return FS_RET_CHECK0(chown (path, owner, group));
}
...
{
        return FS_RET_CHECK0(fchown (fd, owner, group));
}
...
{
        return FS_RET_CHECK0(lchown (path, owner, group));
}
...
{
        return FS_RET_CHECK0(truncate (path, length));
}
...
{
        return FS_RET_CHECK0(ftruncate (fd, length));
}
...
{
        return FS_RET_CHECK0(utimes (filename, times));
}
...
sys_utimensat (int dirfd, const char *filename, const struct timespec times[2],
               int flags)
{
        return FS_RET_CHECK0(utimensat (dirfd, filename, times, flags));
}
#endif
...
{
        return FS_RET_CHECK(readv (fd, iov, iovcnt));
}
...
{
        return FS_RET_CHECK(writev (fd, iov, iovcnt));
}
...
{
        return FS_RET_CHECK(read (fd, buf, count));
}
...
{
        return FS_RET_CHECK(write (fd, buf, count));
}
...
{
        return FS_RET_CHECK(preadv (fd, iov, iovcnt, offset));
}
...
{
        return FS_RET_CHECK(pwritev (fd, iov, iovcnt, offset));
}
...
{
        return FS_RET_CHECK(pread (fd, buf, count, offset));
}
...
{
        return FS_RET_CHECK(pwrite (fd, buf, count, offset));
}
...
{
        return FS_RET_CHECK(lseek (fd, offset, whence));
}
...
{
#ifdef GF_DARWIN_HOST_OS
        return FS_RET_CHECK0(fcntl (fd, F_FULLFSYNC));
#elif __FreeBSD__
	return FS_RET_CHECK0(fsync (fd));
#else
        return FS_RET_CHECK0(fdatasync (fd));
#endif
}
               ...
               size_t size, int flags)
{
        ...
        return FS_RET_CHECK0(lsetxattr (path, name, value, size, flags));
#endif
...
#if defined(GF_LINUX_HOST_OS) || defined(__NetBSD__)
        return FS_RET_CHECK(llistxattr (path, list, size));
#endif

#ifdef GF_BSD_HOST_OS
        return FS_RET_CHECK(extattr_list_link (path, EXTATTR_NAMESPACE_USER,
                            list, size));
#endif

#ifdef GF_SOLARIS_HOST_OS
        return FS_RET_CHECK(solaris_listxattr (path, list, size));
#endif
...
#if defined(GF_LINUX_HOST_OS) || defined(__NetBSD__)
        return FS_RET_CHECK(lgetxattr (path, name, value, size));
#endif
...
#ifdef GF_SOLARIS_HOST_OS
        return FS_RET_CHECK(solaris_getxattr (path, name, value, size));
#endif
...
#if defined(GF_LINUX_HOST_OS) || defined(__NetBSD__)
        return FS_RET_CHECK(fgetxattr (filedes, name, value, size));
#endif
...
#if defined(GF_LINUX_HOST_OS) || defined(__NetBSD__)
        return FS_RET_CHECK0(fremovexattr (filedes, name));
#endif
...
#ifdef GF_SOLARIS_HOST_OS
        return FS_RET_CHECK0(solaris_fremovexattr (filedes, name));
#endif

#ifdef GF_DARWIN_HOST_OS
        return FS_RET_CHECK0(fremovexattr (filedes, name, 0));
#endif
}
...
#if defined(GF_LINUX_HOST_OS) || defined(__NetBSD__)
        return FS_RET_CHECK(flistxattr (filedes, list, size));
#endif

#ifdef GF_BSD_HOST_OS
        return FS_RET_CHECK(extattr_list_fd (filedes, EXTATTR_NAMESPACE_USER,
                                             list, size));
#endif

#ifdef GF_SOLARIS_HOST_OS
        return FS_RET_CHECK(solaris_flistxattr (filedes, list, size));
#endif
...
#if defined(GF_LINUX_HOST_OS) || defined(__NetBSD__)
        return FS_RET_CHECK0(lremovexattr (path, name));
#endif
...
#ifdef GF_SOLARIS_HOST_OS
        return FS_RET_CHECK0(solaris_removexattr (path, name));
#endif

#ifdef GF_DARWIN_HOST_OS
        return FS_RET_CHECK0(removexattr (path, name, XATTR_NOFOLLOW));
#endif
...
{
        return FS_RET_CHECK0(access (pathname, mode));
}
...
sys_fallocate(int fd, int mode, off_t offset, off_t len)
{
        ...
        return FS_RET_CHECK0(fallocate(fd, mode, offset, len));
#endif
        ...
        errno = FS_RET_CHECK_ERRNO(posix_fallocate(fd, offset, len));
        if (errno)
                gf_log ("FS", GF_LOG_ERROR, "posix_fallocate fails with %s", strerror(errno));
        return errno ? -1 : 0;
#endif
        ...
        if (ret == -1)
                return ret;
        return FS_RET_CHECK0(ftruncate (fd, offset + len));
#endif
        errno = ENOSYS;
        return -1;
}
