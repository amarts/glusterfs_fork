...
inode_set_namespace_inode(inode_t *inode, inode_t *ns_inode)
{
    ...
    if (!IA_ISDIR(ns_inode->ia_type)) {
      gf_log(THIS->name, GF_LOG_WARNING, "Trying to link namespace which is not a directory");
        return;
    }

    /* Ideally, if we do this, we should do this to complete tree */
    /* FIXME: fix above once we support setting quota after having data in directories */
